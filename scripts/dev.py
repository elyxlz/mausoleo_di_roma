...
Usage:
    python scripts/dev.py metrics [<date> ...]
...
HTML_OUTPUT = pl.Path("eval/eval_report.html")
...
def cmd_metrics(args: list[str]) -> None:
    ...
    dates = args if args else None
    results = evaluate_all(dates=dates)
    print_results(results)


def cmd_inspect(_args: list[str]) -> None:
...
def _image_to_data_uri(path: pl.Path) -> str:
    data = base64.b64encode(path.read_bytes()).decode()
    return f"data:image/jpeg;base64,{data}"


def _compute_metrics(gt_text: str, pred_text: str) -> dict[str, float]:
    from mausoleo.eval.evaluate import compute_cer, compute_wer

    if not pred_text.strip():
        return {"cer": 1.0, "wer": 1.0}
    return {"cer": compute_cer(gt_text, pred_text), "wer": compute_wer(gt_text, pred_text)}


def cmd_html_report(_args: list[str]) -> None:
    ...
    for pred_path in sorted(PRED_DIR.glob("*.json")):
        ...
        if date not in gt_data:
            continue
        try:
            pred_issue = _load_issue(pred_path)
        except Exception:
            continue
        pred_text = extract_full_text(pred_issue)
        metrics = _compute_metrics(gt_data[date]["text"], pred_text)
        config_metrics[cfg_name].append(metrics)
        config_texts[cfg_name][date] = _issue_to_markdown(pred_issue)
...
function switchTab(el, cfg, date) {
  ...
  if (content) content.classList.add('active');
    ...
    print(f"wrote {HTML_OUTPUT} ({len(html)} bytes)")
