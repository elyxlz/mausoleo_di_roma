...
Spend tracking: every researcher trial + every judge call adds to a
running ``current_spend`` written to ``RUNLOG.md`` after each trial. If
``current_spend`` ever exceeds ``BUDGET_USD`` (default $18 to leave room
for write-up), the runner finishes the in-flight trial cleanly, writes a
PARTIAL marker to ``/tmp/mausoleo_phase2_done.txt``, and exits.
"""
...
BUDGET_USD = 18.0  # leave $2 buffer below the user-set $20 cap
N_TRIALS = 3
...
CASES: list[dict[str, tp.Any]] = [
    ...
    },
    {
        ...
        "question": (
            "How does the balance of war coverage versus domestic-politics coverage "
            "shift over July 1943 in Il Messaggero? Give a quantitative or "
            "structurally-aggregated answer at month scale, citing specific dates."
        ),
        "lead": False,
    },
]
...
def run_all(*, only_cases: tuple[str, ...] | None = None) -> dict[str, tp.Any]:
    ...
    aggregate: dict[str, tp.Any] = {"started": _now(), "trials": []}
        ...
        for system in SYSTEMS:
            for trial in range(1, N_TRIALS + 1):
                if spend.total > BUDGET_USD:
                    _append_runlog(
                        f"BUDGET CAP HIT at ${spend.total:.2f} before {case['case_id']}/{system}/t{trial}; "
                        f"stopping. researcher=${spend.researcher_usd:.2f} judges=${spend.judges_usd:.2f}"
                    )
                    DONE_FLAG.write_text(
                        f"PARTIAL: budget cap hit at ${spend.total:.2f}; "
                        f"stopped before {case['case_id']}/{system}/t{trial}\n"
                    )
                    aggregate["stopped_at_budget"] = True
                ...
                aggregate["trials"].append({
                    ...
                    "researcher_cost_usd": round(u.cost_usd, 5),
                    "trial_elapsed_sec": round(time.time() - trial_t0, 2),
                })
                _append_runlog(
                    f"  done {case['case_id']}/{system}/t{trial}: "
                    f"calls={u.tool_calls} chars={u.chars_read} recall={comp['recall']:.2f} "
                    ...
                    f"spend=${spend.total:.2f} ({u.elapsed_sec}s)"
                )
                # Persist running aggregate after every trial so we never lose
                # data on a crash.
                aggregate["current_spend_usd"] = round(spend.total, 4)
                aggregate["researcher_spend_usd"] = round(spend.researcher_usd, 4)
                aggregate["judges_spend_usd"] = round(spend.judges_usd, 4)
                AGG_PATH.write_text(json.dumps(aggregate, ensure_ascii=False, indent=2, default=str))
    ...
    _append_runlog(
        f"\nfinished at ${spend.total:.2f} researcher=${spend.researcher_usd:.2f} "
        f"judges=${spend.judges_usd:.2f} wall={aggregate['wall_time_sec']:.0f}s"
    )
    return aggregate
...
def compute_per_case_stats(trials: list[dict[str, tp.Any]]) -> dict[str, tp.Any]:
    ...
    for case_id, sysmap in by_case.items():
        ...
        for metric_name, key in (
            ...
            mausoleo_vals = [tr[key] for tr in m]
            baseline_vals = [tr[key] for tr in b]
        ...
        # Sign test on completeness (3 trials, 1 obs per trial since recall
        # is system-not-judge). For case 1, baseline recall is 0 by
        # construction, so we still compute but the result is trivially
        # decisive; we annotate this separately.
