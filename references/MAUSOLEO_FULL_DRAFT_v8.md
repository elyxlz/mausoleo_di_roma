...
Case 2 cleared in roughly twelve Mausoleo tool calls against a baseline that saturated its thirty-call budget every trial, with recall rising from ~six-in-ten to ~three-in-four. Case 1 halves tool calls but ties on touched-set recall (0.67), because that metric is the wrong instrument for an absence; quality still tilts to Mausoleo. Case 3 has the largest quality gap (4.06 vs 3.17) and the lowest κ (0.14), the rubric's narrative bias showing through (§7.2). Sign-test p-values run 0.125 to 1.000; coarse, but the direction is uniform.
...
Four points from §2.3 carry the synthesis. First, memory is hierarchically organised, with episodic traces consolidating into schema-level abstractions over time (Bartlett, 1932). The three case studies map onto this gradient directly: case 1 is episodic (a specific date), case 2 narrative (an integrated arc across episodes), case 3 schematic (a compressed monthly shape). Mausoleo winning on every cell is consistent with the prediction that an index whose levels match the levels at which memory operates will be more usable than one that flattens them. Second, the working-memory chunking constraint (Miller, 1956; Cowan, 2001) explains the *shape* of the win: the baseline saturates its tool-call budget reading article-level material, while Mausoleo finishes in roughly half the calls reading day- or week-level summaries that fit into the active workspace. The efficiency gap is the chunking constraint expressed as a tool-call count.

Third, Eichenbaum (2017) and the Tolman-Eichenbaum-Machine (Whittington et al., 2020) ground the structural claim. The hippocampal machinery that organises spatial navigation is the same machinery that organises temporal sequence and conceptual relation; the brain's representation of time is hierarchically compositional in the same way its representation of space is. A multi-resolution temporal index is therefore not a metaphor on cognition but the same structural form, instantiated externally. The cognitive-architectural fact about how time is represented predicts that an interface exposing that structure should be the right one for an archival research task. Fourth, the extended-mind argument (Clark and Chalmers, 1998) licenses treating the index as part of the cognitive system. When a researcher habitually descends through Mausoleo, the index functions as part of the system the researcher uses; design choices are therefore cognitive choices, not just interface choices. The schema's separation of leaf text and summary text matters in this sense: the researcher offloads chunking work to the index without losing the ability to descend back to the source.
...
The honest caveat is that rigorous testing of the cognitive parallel against human researchers is left to future work. This dissertation tests the design's effect on an LLM-agent proxy: the agent in §6 stands in for the cognitive process the framing describes, and the results bear on the architecture-and-agent system rather than on human researchers directly. The narrower technical claim §6 supports is that calendar-given hierarchies admit absent-date nodes that learned hierarchies cannot. The wider cognitive claim, that the architecture works because it aligns with multi-resolution temporal memory, is the one this section formulates and the one a human-subjects follow-up would test.
...
The system sits in the recent hierarchical-retrieval lineage in NLP, alongside RAPTOR (Sarthi et al., 2024), GraphRAG (Edge et al., 2024) and PageIndex (Zhang and Tang, 2025), and contributes one specific architectural choice: take the source's publication calendar as the retrieval tree rather than inducing the hierarchy from chunk embeddings or community detection. The choice is what makes the absent-26-July node representable, and that property is what drives the case-1 result. §7.3 develops the framing in light of the cognitive-science literature on multi-resolution temporal memory (Bartlett, 1932; Miller, 1956; Eichenbaum, 2017; Whittington et al., 2020) and on extended cognition (Clark and Chalmers, 1998); the engineering substance is in §3 to §6. The substantive interdisciplinary contribution is the demonstration that respecting cognitive-architectural intuitions about temporal hierarchy yields measurably better archival interfaces.

Several directions extend the work. The most important is human-subjects testing of the cognitive parallel: the present evaluation tests the design's effect on an LLM-agent proxy, and a study with working historians would bear directly on the cognitive claim §7.3 formulates. Scaling beyond a single month is the next immediate direction; OCR scales roughly linearly while the higher index tiers grow sub-linearly, so a multi-decade or multi-archive run is engineering rather than research. Federated retrieval across heterogeneous archives, each with its own provenance conventions, would need a richer node model than the present single-source one. A community-correction workflow over the summariser-activation layer is a third direction the schema already supports: editorial corrections to summaries do not corrupt the leaves, because source and activation sit in distinct columns of the same row. None of these would change the basic design commitment. Mausoleo is one interface that satisfies it. The wider claim, which the dissertation makes empirically rather than rhetorically, is that designing archival-research interfaces around the cognitive architecture researchers bring to historical material is the right starting point for the field.
