...
A caveat on scope: rigorous testing of the cognitive parallel against working historians remains for future work. The §6 evaluation uses an LLM-agent proxy in place of the cognitive process the framing describes, so the results bear on the architecture-and-agent system rather than on human researchers directly. The narrower technical claim §6 supports is that calendar-given hierarchies admit absent-date nodes that learned hierarchies cannot. The wider cognitive claim, that the architecture works because it aligns with multi-resolution temporal memory, is the one this section formulates and the one a human-subjects study would test.
