...
The W29 summariser prolepsis flagged in §5.3 ("*il giorno successivo, 25 luglio, avverrà l'arresto di Mussolini*") is the place this background actually bites the engineering. The summary at week level adds historical context the source paragraphs could not contain, which is interpretive activation rather than compression; §7.2 logs it as a limitation on summariser bias. The schema's separation of leaf text and summary text means the original paragraphs remain reachable for source-critical reading, but the bias-quantification step (human-written reference summaries at day, week, month) is out of scope for this dissertation and is the obvious next piece of work for any deployment beyond the case-study month.

The narrower technical claim, which §6 supports directly, is that calendar-given hierarchies admit absent-date nodes that learned hierarchies cannot, and that this is a measurable property of the architecture rather than a rhetorical flourish about it.
