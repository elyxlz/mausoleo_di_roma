...
I thank Dr Yi Gong for supervising; her comments on §2.3 and §7.2 shaped how the archival-science material sits next to the OCR and case-study chapters.
...
Mausoleo is a hierarchical retrieval pipeline for digitised historical newspapers, evaluated here on a single demanding month: *Il Messaggero*, July 1943. Thirty surviving issues, around 6,480 hand-cleaned article transcriptions, are organised into five summary tiers (paragraph, article, day, week, month) inside a single ClickHouse `nodes` table, with an LLM agent traversing the tree through a small typer-based CLI. The benchmark used throughout is a BM25 retriever over the same article transcriptions, with the comparison drawn on tool-call efficiency, completeness against a hand-built relevance ground-truth, and judge-rated quality on a three-dimension rubric. Three case studies cover the absent issue of 26 July 1943, the editorial register shift across the regime change of 25 to 27 July, and the per-week balance of war versus domestic-politics coverage across the month.
...
The central design commitment is that the hierarchy is given, not learned. RAPTOR clusters chunk embeddings recursively to induce a tree (Sarthi et al., 2024); GraphRAG extracts an entity-relation graph and community-detects a hierarchy over it (Edge et al., 2024); Mausoleo does neither, taking the corpus's publication calendar as the tree. The cost is the loss of latent thematic structure that cuts across time; the embedding-search endpoint (§3.3) partially recovers it. The benefit on this kind of source is that an absent date is representable as a node, which the case study in §6.2 turns into a measured capability gap relative to the BM25 baseline.
...
The Mausoleo agent reaches this node in 13.3 tool calls on average (range 11 to 15), and its compiled answers score a judge mean of 4.56 (range 4.00 to 5.00) against the baseline's 4.22. Recall against the article-id GT is 0.67, tied at the mean with the baseline; trial-to-trial dispersion is diagnosed in `references/case_1_variance_note.md`. Judges disagree at κ = 0.33; the paired sign test on quality (n = 6) returns p = 0.625, so the win is structural and not statistically decisive at this sample size. The recall tie at 0.67 is itself the symptom: touched-articles is the wrong instrument when the answer is an absent issue. The case turns on the architecture: date is a first-class structural property of the index, an absent date instantiates a node, the node carries a summary, and the absence becomes addressable. A flat system would need ad-hoc out-of-band metadata to recover the same affordance.
...
The narrower technical claim, which §6 supports directly, is that calendar-given hierarchies admit absent-date nodes that learned hierarchies cannot. This is a measurable architectural property and not a rhetorical one.
