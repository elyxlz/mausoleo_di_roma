...
A note on background and scope. I came to the project from machine learning coursework rather than from history; my Italian is conversational, sufficient for reading regime-era press but not for translating the *MinCulPop veline*, which I worked from in Murialdi's transcription. The 1885 *Il Messaggero* evaluation issue I hand-transcribed turned out to be harder than the 1910 one, partly because the typography is denser and partly because I did not initially realise the column-six serialised fiction was set in a different font and had a separate header — I redid that issue, and the page_accuracy floor of 0.683 reported in §4.1 is, embarrassingly, an artefact of an annotation error in my own transcription that I caught too late to redo without invalidating the hill-climb trajectory. I thank Dr Yi Gong for supervising; her comments on §2.3 and §7.2 shaped how the archival-science material sits next to the OCR and case-study chapters. Conversations with two MSc Computational Statistics students in the Foster Court common room about the case-3 oracle metric design were also useful.
...
Cost: the one-time OCR build took ~29.7 GPU-hours on 2× RTX 3090; the LLM index build (Haiku 4.5 for 6,480 article summaries, Sonnet 4.5 for 37 higher-tier ones) was a $28.87 USD-equivalent figure against the Claude Max OAuth quota. Per trial Mausoleo used 328k input tokens at 76.7 s wall; baseline 321k at 81.6 s. ClickHouse over 6,517 nodes returns sub-second; per-query cost is dominated by the LLM, not the index.
