...
A digitised newspaper corpus normally allows a historian to retrieve articles by keyword, with date as a facet on the side. For the July 1943 *Il Messaggero* corpus this dissertation works with, that template handles a question for which articles exist and falters on the others the corpus invites: where there are no articles to retrieve, where the unit of interest is a few-day editorial trajectory, where the unit is a month-long aggregate. A flat article index returns nothing on the 26 July gap (the day after the Grand Council deposed Mussolini, when the morning paper did not appear), several articles on the regime-change trajectory of 25 to 27 July that have to be assembled by the reader, and several hundred articles on the war-and-domestic balance question that have to be classified by hand. The dissertation argues that a digital archive ought to respond to questions of these shapes with the structural information available to it, and builds a system, Mausoleo, that does so by storing the corpus as a calendar-shaped tree of recursively summarised nodes over which a researcher agent navigates.

There is an interdisciplinary argument running underneath this. A century of cognitive-science work has accumulated evidence that memory organises temporal information at multiple resolutions, with the everyday consequence that researchers reading time-stamped material shift between date-bound items and the larger schemas they build up over weeks. A flat ranked-article interface asks the researcher to perform that integration mentally. The case studies in chapter four ask whether exposing the temporal structure in the index changes how the work goes. Three questions are put to *Il Messaggero* in July 1943: what the paper said on the absent 26 July, how it covered the regime change of 25 to 27 July, and how the balance of war and domestic-politics coverage moved across the month. Across eighteen scored trials the system averaged 11.3 tool calls against the keyword baseline's 28.3 and was preferred by both judges on a three-dimension rubric in every case. The calendar-shaped index holds a day node for 26 July with empty leaves and a summary that records the absence as evidence, where a flat article index has no equivalent slot for that day; this missing-day case is the test the design was built around.

---

## Preface

I came to this project from the engineering side, after a year working on retrieval-augmented generation pipelines for technical documentation. I wanted a final-year project that would put the same techniques in front of source material that does not chunk well, and Italian regime-period newspapers were the natural pick from the languages I read. The 1943 issue list of *Il Messaggero* was the most complete in the available scans, which is how I ended up with a corpus that contained one missing day. Bartlett's *Remembering* was a paper I had read for second-year psychology and went back to once the OCR work began producing day-summary nodes; the hippocampal-mapping work in Eichenbaum and Whittington came in much later, after the calendar-shaped index had already been built.
...
The simplest description of what this dissertation does is that it asks how a researcher reading a month of an Italian regime-period daily, *Il Messaggero* in July 1943, gets at three kinds of question that the existing digitised newspaper interfaces handle badly: a question about a single day, a question about a short editorial trajectory across three days, and a question about how war and domestic-politics coverage balance across the whole month. The first of these is sharper than it sounds because the day in question is one for which no issue exists.

That day is 26 July 1943. The 25 July morning edition went to press long before the Grand Council vote that night and in its accustomed late-fascist register. The 27 July edition reappeared under the new government, with the Grand Council *ordine del giorno*, the King's proclamation, and the new ministry on the front page. The 26 July does not appear in the digitised fund. Mussolini had been deposed and arrested overnight; an editorial line for the morning paper could not be drawn in time and the issue did not run.
...
Mausoleo extends this lineage by changing the source of the hierarchy. The prior systems above apply hierarchical retrieval either to a single document's section structure or to a clustering induced from chunk embeddings, while Mausoleo applies the same paradigm to an archival corpus whose hierarchy is given by chronology rather than induced from the data. Daily newspapers already carry a temporal hierarchy in their own production schedule, and constructing the index over the calendar lets the original article-to-issue-to-day chain stay intact without extra annotation work.
...
When the catalogue holds only article-shaped slots, a day with no articles drops out of what the corpus shows. *Il Messaggero* of 26 July went unprinted because the editorial line of a regime-aligned daily could not be drawn in the twenty-four hours between deposition and the new government's first announcement, and the catalogue covering a corpus of this source-critical kind needs to carry the slot the source itself left empty.
