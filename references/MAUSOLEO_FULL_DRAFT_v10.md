...
There is an interdisciplinary argument running underneath this. A century of cognitive-science work has accumulated evidence that memory organises temporal information at multiple resolutions, and that researchers reading time-stamped material rely on that organisation when moving between date-bound items and the larger schemas they build up over weeks. A flat ranked-article interface asks the researcher to perform that integration mentally. The case studies in chapter four ask whether exposing the temporal structure in the index changes how the work goes. Three questions are put to *Il Messaggero* in July 1943: what the paper said on the absent 26 July, how it covered the regime change of 25 to 27 July, and how the balance of war and domestic-politics coverage moved across the month. The calendar-shaped index holds a day node for 26 July with empty leaves and a summary that records the absence as evidence, where a flat article index has no equivalent slot for that day. Quantitative results sit in chapter four.
...
The recent hierarchical-retrieval lineage breaks with this assumption. RAPTOR (Sarthi et al., 2024) builds a retrieval tree by recursively clustering chunk embeddings and summarising each cluster, allowing the same query to hit a leaf passage or a higher-level summary depending on its scope. GraphRAG (Edge et al., 2024) takes a different route and extracts an entity-relation graph from the corpus, runs Leiden community detection over the graph, and writes summaries at each community level. The Edge et al. paper reports substantial gains over flat retrieval-augmented generation for global-summarisation queries on the benchmarks they ran (I have not been able to replicate them locally on the Italian corpus, in part because Leiden detection over an Italian-language entity graph is its own engineering problem). The cost the paper itself acknowledges is that the entity-extraction pass and the community summarisation are both expensive at scale, and the resulting hierarchy is opaque to the user: a researcher cannot tell from the surface why two summaries sit at the same level. A related line uses the document's own table-of-contents as the retrieval target (Zhang and Tang, 2025) and so reads its hierarchy off the surface rather than inducing one. The hierarchies these systems propose are induced from the data or read off a document's structure; in either case the retriever has more to work with than a flat list.
