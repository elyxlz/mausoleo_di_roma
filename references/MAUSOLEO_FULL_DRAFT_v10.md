...
Mausoleo changes the source of the hierarchy again. Daily newspapers carry a temporal hierarchy in their production schedule (paragraph in article in issue in day in week in month), and the index inherits that structure rather than inducing one. The advantage is concrete on a corpus where the source-side hierarchy is already meaningful: no clustering decisions, no quality-of-extracted-graph dependence, and a navigation surface a working historian can predict.
