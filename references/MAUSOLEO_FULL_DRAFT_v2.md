...
This dissertation presents Mausoleo, an end-to-end pipeline that combines an ensemble VLM OCR stage, a chronologically organised five-level summary index stored in ClickHouse, and an agent-mediated search interface over that index. The hierarchy is the calendar itself; the system never clusters articles to produce one. Two commitments fall out of this choice for free, both already present in the historiographical and archival traditions: multi-resolution time, and respect for provenance.

The pipeline is evaluated on *Il Messaggero*, July 1943, against a BM25 baseline operating over the same article corpus. Across three case studies and eighteen scored trials, Mausoleo answered in roughly eleven tool calls per trial against the baseline's twenty-eight, and was preferred by both LLM judges on a three-dimension rubric in every case tested. Quality margins, judge means and inter-judge agreement are reported case-by-case in §6.5. The most consequential case concerns the issue of 26 July 1943, the day after the deposition of Mussolini, which is missing from the source archive: Mausoleo represents the absent day as a first-class node whose summary contextualises the gap, while the BM25 baseline can only reason about the absence from sibling-day articles, with no node in the index that grounds the missing day.

The contribution is the empirical demonstration: a chronologically given, provenance-respecting hierarchy is the right interface form for this kind of archival research. The case studies in §6 are what the claim rests on.
...
The historiographical contribution to the design is the Annales commitment to multi-resolution time. Day, week, month and year are how the historian working on a regime period already thinks; they are not aggregation choices the engineer is free to invent. Once a system commits to that grain, the question of which dates exist and which do not stops being a metadata footnote and becomes part of the index proper. The missing 26 July is, on this reading, archivally significant exactly because it is the gap in the provenance — and provenance, in the sense Cook (2013), Ketelaar (2001) and Schellenberg (1956) carved out across half a century of archival theory, is what insists that records be respected in their original order and that description is activation of the source rather than replacement of it. (Archival science is in this sense subordinated to the historiography here: it supplies the discipline that prevents the engineering from rearranging the source for its own convenience.)

Computer Science contributes the recent hierarchical-retrieval lineage. RAPTOR, GraphRAG and PageIndex are the obvious priors. Without this lineage there is no interface.

The substantive integration sits in §7.3, where I argue that Mausoleo's chronologically-given hierarchy carries, by construction, the two commitments that archival history has long shared. The case studies in §6 are how the argument is tested.

The two disciplines are paired here because the question genuinely needs both. A historiography dissertation about Annales reading practice across the regime change would not need to build a search interface, and a CS dissertation on hierarchical retrieval would have no particular reason to make the hierarchy chronological. Working at the intersection turns out to mean writing prose that has to satisfy two reading audiences who do not normally read each other; some of the awkwardness in §2.2 and §7.3 is downstream of that. Earlier BASc modules in historical methodology and in machine learning supplied the working vocabulary. I thank Dr Yi Gong for supervising; her comments on §2.3 and §7.2 shaped how the archival-science material is held against the engineering one. Cognitive science, philosophy, and media studies are out of scope.
...
Two kinds of question make the gap concrete. Aggregate questions ("how did war coverage shift across July 1943?") force the researcher to read many articles, classify them, and reconstruct the distribution by hand under flat keyword retrieval. Missing-data questions ("what was reported on 26 July 1943?") flat retrieval cannot answer at all: the date returns no results, and the silence is indistinguishable from the absence of relevant matches. Both want either an indexed hierarchy of summaries with day, week and month as first-class objects, or considerable manual aggregation across point queries.

This dissertation proposes Mausoleo, an OCR plus hierarchical-indexing plus agent-mediated search pipeline, as a computational form for the multi-resolution reading that Annales-school historiography already practises by hand. The architecture is a five-level chronological tree (paragraph, article, day, week, month) over the thirty surviving July 1943 issues of *Il Messaggero*, written into a ClickHouse `nodes` table with a recursively summarised parent at every level and an LLM agent that drills the tree through a small set of CLI tools. The methodological lineage is hierarchical retrieval, with RAPTOR and Murugaraj et al.'s (2025) Topic-RAG over the Impresso corpus as the closest priors and GraphRAG as the comparator from a different family of induction; §2.2 develops the comparison. Mausoleo's move against this lineage is methodological: its hierarchy is given by archival chronology rather than induced from the data, and it treats the calendar of an archival corpus as the tree, with absent dates handled architecturally as nodes whose summaries contextualise the gap.
...
The case studies in §6 measure efficiency, completeness, and quality across three trials per system per case, scored by two LLM judges. The absent-day node is a first-class object in the index rather than a metadata facet (§3.2 sets out the design that makes this possible), and §7.1 returns to it as the case where the architecture has to be doing several things at once for the absence to be addressable. Multi-resolution time, a long-standing commitment in the historiography, turns out to be readily encodable as an interface; Mausoleo is one such encoding.
...
The closest prior work on RAG over historical newspapers specifically is Murugaraj et al. (2025), who apply a topic-restricted retrieval pipeline (Topic-RAG) to the Impresso Swiss corpus and report improved retrieval relevance over flat RAG, measured by BERTScore, ROUGE and UniEval. Their pipeline restricts retrieval to documents matching the query's inferred topic, on the assumption that topical coherence between query and retrieved articles is the dominant signal of relevance. The disagreement I want to record is with that assumption, in the case where the corpus has a strong native temporal hierarchy. For a question like "what shifted across the regime collapse?", the topically most-relevant articles will cluster around the surface vocabulary of fascism — Mussolini, Grand Council, Badoglio — and the system will return them at a uniform editorial register that flattens the very shift the question is asking about. Murugaraj et al.'s evaluation on BERTScore-style metrics misses this, because those metrics reward semantic overlap between answer and reference and are blind to whether the answer captures a temporal trajectory at all. Topic-RAG and Mausoleo are answering different questions, in other words: theirs is "given a query, retrieve the topically relevant articles"; mine is "given a temporal slice, return what the slice looked like, including its absences". The Impresso comparison is the one I would most like to run head-to-head, but the corpus is in French and German and the case studies in §6 require Italian-language summarisation; that is a scope cost I have not been able to absorb within this dissertation.
...
Read against one another, these literatures fix the position Mausoleo occupies. The newspaper archives are the access modality it extends. The IR lineage validates the paradigm and locates the specific move (calendar-given hierarchy in place of induced clustering); the historiography then supplies the reason that move is appropriate at all for an archival corpus. What is left is to build the system, which is what the rest of the dissertation does.
...
The central design commitment is that the hierarchy is given, not learned. RAPTOR clusters chunk embeddings recursively to induce a tree (Sarthi et al., 2024); GraphRAG extracts an entity-relation graph and community-detects a hierarchy over it (Edge et al., 2024); Mausoleo does neither. The tree is the calendar. This loses the ability to surface latent thematic structure cutting across time, which the embedding-search endpoint partially recovers (§3.3), but the gains for archival research are two: provenance is preserved by construction, since every summary points down to its source paragraphs and up to chronological context; and the structure is legible to a historian without training, because the tree mirrors how the source is already organised.
...
A second-order limitation cuts deeper, and is the one that worried me most when I wrote §5.3. Every summary at every level is the LLM's choice of what is salient, and the W29 prolepsis ("*il giorno successivo, 25 luglio, avverrà l'arresto di Mussolini*") is summariser activation rather than source content. I cannot pursue the bias-quantification within the scope of this dissertation: building human-written reference summaries at the day, week and month level for July 1943 alone would be a separate undertaking of weeks. The honest position is that the activation layer is doing more interpretive work than I have been able to audit here. Further limitations follow. The evaluation runs on a single corpus and a single politically-volatile month; July 1943 was in part chosen because the rupture is so legible, and less volatile corpora may not behave the same way. Both judges are Anthropic models, distinguished only by system prompt; the κ values in §6.5 are therefore intra-vendor, and cross-vendor robustness is untested. The three-dimension rubric was designed for narrative completeness and fits the case-3 aggregate-shape answer poorly (κ = 0.14). Relevance ground truth is single-annotator, and the planned 2-week intra-rater self-consistency check was not performed within the dissertation timeline; second-annotator inter-rater agreement is the obvious next step. Three trials per case make the paired sign test appropriate but do not bound effect-size estimation. Finally, the architecture assumes a strong native temporal hierarchy in the source: archives without dated issues, such as unpublished correspondence or undated collections, need a different organising principle and are out of scope.

## §7.3 The Annales hierarchy and provenance, in one tree

When the index is built over a calendar rather than over a learned clustering, the hierarchy that appears is the same one the archive already had: chronological, source-given, and addressable at any level the historian might read at. That is the synthetic claim the dissertation argues for. The disagreement it sits across is specific. Computational retrieval, in its hierarchical-RAG form (RAPTOR; GraphRAG), treats hierarchy as something to be induced from the data, on the assumption that the relevant structure is latent. Archival science, by contrast, treats hierarchy as something to be respected from the source: provenance pre-exists any system that reads it (Cook, 2013; Schellenberg, 1956). Historians of the press routinely resist algorithmic salience filters as a category of source distortion. The chronological hierarchy of newspaper archives is one place these traditions can be reconciled, because chronology is at once a property the source already has and a structure the system can compute over without inducing it.
...
An interface to a historical archive needs to respect the historiographical commitment to multi-resolution time. It also needs to respect the archival commitment to provenance. Mausoleo's chronological hierarchy is Braudelian stratification in one register and Schellenbergian original order in the other; the summaries are activations in the sense Cook and Ketelaar use the word. The two readings of the same tree do not need to be reconciled because the tree was never optimised for either of them; chronology is the substrate they both happen to be commitments about. Flat retrieval discards the stratification and treats provenance as a metadata facet; no node in a flat index can carry the missing 26 July as a first-class object. Architecture and principle meet most legibly there.
...
The contribution is interdisciplinary in a substantive sense. Archival research is a humanities task whose constitutive commitments — multi-resolution time in the Annales tradition, respect for provenance in archival science — precede any particular tool. The system itself is a CS artefact: a hierarchical-retrieval architecture in the RAPTOR-and-GraphRAG lineage, but with the tree given by the source's calendar rather than induced from chunk embeddings. §7.3 develops the argument that, on this kind of source, the two literatures' commitments fall out of the same design choice and do not need to be balanced against one another. The missing 26 July is where the architecture and the principle line up most clearly, and that is the case the chapter leads with.

Several directions extend the work. Scaling beyond a single month is the most immediate; OCR scales roughly linearly while the higher index tiers grow sub-linearly, so a multi-decade run is engineering rather than research. Federated retrieval across heterogeneous archives — each with its own provenance conventions — would need a richer node model than the present single-source one. A community-correction workflow over activation layer is a third direction the schema already supports: editorial corrections to summaries do not corrupt the leaves, because source and activation sit in distinct columns of the same row. None of these would change the basic design commitment. Mausoleo is one interface that satisfies it. The wider claim, which the dissertation makes empirically rather than rhetorically, is that the commitment itself is the right starting point for archival-research interfaces.
